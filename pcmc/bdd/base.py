"""pcmc-bot / bdd / Briques de connection
...
class TableMeta(declarative.api.DeclarativeMeta):
    """Métaclasse des tables de données du PCMC-Bot.
    ...
    def find_nearest(cls, chaine, col=None, sensi=0.25, filtre=None,
        ...
        Raises:
            ...
            ~ready_check.NotReadyError: session non initialisée
                (:attr:`.pcmc.config.session` vaut ``None``)
...
def autodoc_Column(*args, doc="", comment=None, **kwargs):
    ...
    if py_type.__module__ in ("builtins", "pcmc.bdd.enums"):
        py_type_str = py_type.__name__
    elif py_type.__module__.startswith("pcmc.bdd.model_"):
        py_type_str = f"pcmc.bdd.{py_type.__name__}"
    else:
...
def connect():
    ...
    - Utilise la variable d'environment ``PCMC_DATABASE_URI``
    - Crée les tables si nécessaire
    - Prépare :obj:`.config.engine` et :obj:`.config.session`
    """
    PCMC_DATABASE_URI = env.load("PCMC_DATABASE_URI")
    # Moteur SQL : connexion avec le serveur
    config.engine = sqlalchemy.create_engine(PCMC_DATABASE_URI,
                                             pool_pre_ping=True)
