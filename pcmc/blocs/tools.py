"""pcmc-bot / blocs / Outils divers et variés
...
# Appel à un admin
def mention_admin(arg):
    """Renvoie la mention ou le nom du rôle admin

        - Si le joueur n'est pas un admin, renvoie la mention de
          :attr:`config.Role.admin`
    ...
    if (isinstance(member, discord.Member)
        and member.top_role >= config.Role.admin):
        # Pas un webhook et (au moins) admin
        return f"@{config.Role.admin.name}"
    else:
        return config.Role.admin.mention


def escape_mentions(text):
    """Remplace les mentions d'objets Discord par leur représentation.

    Représentations utilisées : ``#salon``, ``@Rôle``,
    ``@Surnom ou nom``, ``:emoji:``).
    ...
    """
    text = re.sub("<#([0-9]{18})>",
                  lambda mt: "#" + channel(mt.group()).name, text)
    text = re.sub("<@&([0-9]{18})>",
                  lambda mt: "@" + role(mt.group()).name, text)
    text = re.sub("<@!([0-9]{18})>",
                  lambda mt: "@" + member(mt.group()).display_name, text)
    text = re.sub("<:(.*):[0-9]{18}>", ":\\1:", text)
    return text
...
#: commande exécutable uniquement par un :attr:`admin <.config.Role.admin>`
#: ou un webhook (tâche planifiée)
admins_only = commands.check_any(
    commands.check(lambda ctx: ctx.message.webhook_id),
    commands.has_role(config.Role.get_raw("admin"))        # nom du rôle
)
...
async def wait_for_message(check, trigger_on_commands=False, chan=None):
    ...
    if chan:
        ...
        warnings.warn("pcmc.tools.wait_for_message called with `chan=None`, "
...
async def sleep(chan, tps):
    ...
    ("*PCMCBot est en train d'écrire...*") sur un salon.
